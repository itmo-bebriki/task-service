...
internal sealed class SubmissionJobTaskSubmissionHandler : IEventHandler<SubmissionJobTaskEvent>
{
    private readonly IKafkaMessageProducer<JobTaskSubmissionKey, JobTaskSubmissionValue> _producer;

    public SubmissionJobTaskSubmissionHandler(IKafkaMessageProducer<JobTaskSubmissionKey, JobTaskSubmissionValue> producer)
    {
        _producer = producer;
    }

    public async ValueTask HandleAsync(SubmissionJobTaskEvent evt, CancellationToken cancellationToken)
    {
